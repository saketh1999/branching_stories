...
export interface ImageEditState {
  ...
  currentImageUrl: string; // This will be updated if an image is regenerated within the dialog session
  ...
  isModified: boolean; // True if prompt or context changed by user for this image
}
...
const EditPanelDialog: FC<EditPanelDialogProps> = ({ 
  ...
  useEffect(() => {
    if (isOpen && panelToEdit) {
      const initialStates: ImageEditState[] = panelToEdit.imageUrls.map((imgUrl, index) => ({
        ...
        currentImageUrl: imgUrl, // Initially same as original
        currentPrompt: panelToEdit.promptsUsed?.[index] || '',
        contextImageUrlForAI: imgUrl, // Default context is the image itself
        contextImagePreviewUrl: imgUrl,
        isModified: false,
      }));
      setImageStates(initialStates);
    } else {
      setImageStates([]); // Reset on close
    }
  }, [isOpen, panelToEdit]);
  ...
  const handleClearCustomContext = (stateIndex: number) => {
    setImageStates(prev => prev.map((state, idx) => 
      idx === stateIndex ? { 
        ...state, 
        contextImageUrlForAI: state.originalImageUrl, // Revert to original image as context
        contextImagePreviewUrl: state.originalImageUrl,
        sourcePanelId: undefined,
        sourceImageIndex: undefined,
        isModified: true // Marking as modified if they explicitly revert context
      } : state
    ));
  };
  ...
  const handleSaveChanges = async () => {
    ...
    const regenerationPromises = modifiedImageUpdates.map(update => 
      ...
      .catch(error => {
        ...
        return { ...update, newImageUrl: imageStates[update.imageIndex].currentImageUrl }; // Keep old image on error for this specific one
      })
    );
    ...
    try {
      ...
      onClose();
    } catch (error) { // This catch might not be strictly necessary if individual promises handle errors
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
        <DialogContent className="sm:max-w-2xl md:max-w-4xl lg:max-w-5xl bg-card">
          <DialogHeader>
            <DialogTitle className="text-2xl text-primary">Edit Panel: {panelToEdit.title || `Panel ${panelToEdit.id.substring(0,4)}`}</DialogTitle>
            <DialogDescription>
              Modify prompts and context images for each image in this panel. Click "Apply Changes" to regenerate modified images.
            </DialogDescription>
          </DialogHeader>
          
          <ScrollArea className="max-h-[calc(70vh - 150px)] p-1">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
              {imageStates.map((state, index) => (
                <div key={state.id} className="p-3 border border-border rounded-md bg-background/50 space-y-3">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 items-start">
                    <div className="w-full sm:w-1/3 aspect-square relative bg-muted rounded border overflow-hidden shrink-0">
                      <Image src={state.currentImageUrl} alt={`Current image ${index + 1}`} layout="fill" objectFit="contain" data-ai-hint="panel image large"/>
                       {state.isModified && (
                        <div className="absolute top-1 right-1 bg-primary/80 text-primary-foreground text-xs px-1.5 py-0.5 rounded-full flex items-center">
                          <RefreshCcw size={12} className="mr-1"/> Modified
                        </div>
                      )}
                    </div>
                    <div className="flex-grow space-y-2 w-full sm:w-2/3">
                      <div>
                        <Label htmlFor={`edit-prompt-${index}`} className="text-sm text-muted-foreground">Prompt</Label>
                        <Textarea
                        ...
                        {state.originalPrompt && state.originalPrompt !== state.currentPrompt && (
                            <p className="text-xs text-muted-foreground mt-1">Original: "{state.originalPrompt.substring(0,30)}..."</p>
                        )}
                      </div>
                      <div>
                        <Label className="text-sm text-muted-foreground">Context Image for AI</Label>
                        <div className="flex items-center gap-2 mt-1">
                          <div className="w-16 h-12 relative bg-muted rounded border overflow-hidden shrink-0">
                            <Image src={state.contextImagePreviewUrl} alt={`Context for image ${index + 1}`} layout="fill" objectFit="contain" data-ai-hint="context preview small" />
                          </div>
                          <div className="flex flex-col gap-1">
                            <Button onClick={() => handleOpenSetContextDialog(index)} variant="outline" size="sm" className="text-xs">
                        ...
                        {state.sourcePanelId && (
                             <p className="text-xs text-muted-foreground mt-1">
                                Using image { (state.sourceImageIndex ?? 0) + 1} from panel <span className="font-medium truncate max-w-[60px] inline-block align-bottom" title={allPanels.find(p=>p.id === state.sourcePanelId)?.title}>{allPanels.find(p=>p.id === state.sourcePanelId)?.title?.substring(0,10) || state.sourcePanelId.substring(0,4)}...</span>
                            </p>
                        )}
                         {state.contextImageUrlForAI !== state.originalImageUrl && !state.sourcePanelId && (
                             <p className="text-xs text-muted-foreground mt-1">Using uploaded context.</p>
                         )}
                         {state.contextImageUrlForAI === state.originalImageUrl && (
                             <p className="text-xs text-muted-foreground mt-1">Using its original image as context.</p>
                         )}
                      </div>
                    </div>
                  </div>
          ...
          <DialogFooter className="mt-6">
            ...
            >
              ...
              Apply Changes & Regenerate ({modifiedCount})
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
