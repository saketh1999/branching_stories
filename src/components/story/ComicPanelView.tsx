...
const ComicPanelView: FC<ComicPanelViewProps> = ({ panel, allPanels, onGenerateNext, onBranch, onUpdateTitle, onRegenerateImage, onEditPanel }) => {
      ...
      <>
        ...
        <ul className="list-disc pl-3 sm:pl-4 space-y-0.5">
          {panel.promptsUsed.map((prompt, index) => (
            <li key={index} className="text-[10px] sm:text-xs"><strong>Img {index + 1}:</strong> {prompt}</li>
          ))}
        </ul>
      </>
    );
  ...
  if (panel.isGroupNode) {
    ...
    return (
      <Card className="w-full h-full flex flex-col bg-card shadow-lg border-primary border-2">
          ...
          <div className="flex items-start justify-between gap-1 sm:gap-2">
             {isEditingTitle ? (
              <div className="flex-grow flex items-center gap-1">
                <Input type="text" value={editingTitleValue} onChange={(e) => setEditingTitleValue(e.target.value)} onKeyDown={handleTitleKeyDown} onBlur={handleTitleEditSubmit} autoFocus className="h-7 sm:h-8 text-sm w-full"/>
                <Button variant="ghost" size="icon" onClick={handleTitleEditSubmit} className="h-6 w-6 sm:h-7 sm:w-7 shrink-0"><Check className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-green-500" /></Button>
                <Button variant="ghost" size="icon" onClick={() => { setIsEditingTitle(false); setEditingTitleValue(panel.title || (panel.userDescription || `Comic Book ${panel.id.substring(0,4)}`));}} className="h-6 w-6 sm:h-7 sm:w-7 shrink-0"><X className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-red-500" /></Button>
              </div>
            ) : (
              <CardTitle className="text-sm sm:text-base md:text-lg font-semibold text-primary truncate cursor-pointer hover:underline flex-grow" title={`Click pencil to edit. ${displayTitle}`} onClick={() => setIsEditingTitle(true)}>
                <BookText className="inline-block mr-1 sm:mr-2 h-4 w-4 sm:h-5 sm:w-5 align-text-bottom"/>{displayTitle}
              </CardTitle>
            )}
            <div className="flex items-center shrink-0">
              {!isEditingTitle && (
                <TooltipProvider><Tooltip delayDuration={100}><TooltipTrigger asChild><Button variant="ghost" size="icon" onClick={() => setIsEditingTitle(true)} className="h-6 w-6 sm:h-7 sm:w-7"><Edit3 className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-muted-foreground" /></Button></TooltipTrigger><TooltipContent side="top"><p>Edit Comic Book Title</p></TooltipContent></Tooltip></TooltipProvider>
              )}
               {panel.userDescription && (
                <TooltipProvider><Tooltip delayDuration={300}><TooltipTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6 sm:h-7 sm:w-7"><Info className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-muted-foreground" /></Button></TooltipTrigger><TooltipContent className="max-w-[180px] xs:max-w-[200px] sm:max-w-xs break-words p-1.5 sm:p-2" side="top">{descriptionContent}</TooltipContent></Tooltip></TooltipProvider>
              )}
            </div>
          </div>
        </CardHeader>
        ...
        <CardFooter className="p-1.5 sm:p-2 md:p-3 grid grid-cols-1 sm:grid-cols-2 gap-1.5 sm:gap-2 border-t mt-auto flex-shrink-0">
  ...
  // Width and height for this card are now controlled by the style prop from ReactFlowNode / FlowchartDisplay
      ...
      <Card className={cn("w-full h-full overflow-hidden bg-card shadow-md flex flex-col border-secondary border-2")}>
         <CardHeader className="p-1.5 sm:p-2">
          <div className="flex items-center justify-between gap-1">
            {isEditingTitle ? (
              <div className="flex-grow flex items-center gap-0.5 sm:gap-1">
                <Input type="text" value={editingTitleValue} onChange={(e) => setEditingTitleValue(e.target.value)} onKeyDown={handleTitleKeyDown} onBlur={handleTitleEditSubmit} autoFocus className="h-6 sm:h-7 text-[11px] sm:text-xs w-full"/>
                 <Button variant="ghost" size="icon" onClick={handleTitleEditSubmit} className="h-5 w-5 sm:h-6 sm:w-6 shrink-0"><Check className="h-3 w-3 text-green-500" /></Button>
                <Button variant="ghost" size="icon" onClick={() => { setIsEditingTitle(false); setEditingTitleValue(panel.title || `Page ${panel.pageNumber}`);}} className="h-5 w-5 sm:h-6 sm:w-6 shrink-0"><X className="h-3 w-3 text-red-500" /></Button>
              </div>
            ) : (
              <CardTitle className="text-[11px] sm:text-xs font-semibold text-secondary-foreground truncate flex-grow cursor-pointer hover:underline" title={`Click pencil to edit. ${pageTitle}`} onClick={() => setIsEditingTitle(true)}>
                 <FileImage className="inline-block mr-0.5 sm:mr-1 h-3 w-3 align-text-bottom"/>{pageTitle}
              </CardTitle>
            )}
            <div className="flex items-center shrink-0">
             {!isEditingTitle && (
                <TooltipProvider><Tooltip delayDuration={100}><TooltipTrigger asChild><Button variant="ghost" size="icon" onClick={() => setIsEditingTitle(true)} className="h-5 w-5 sm:h-6 sm:w-6"><Edit3 className="h-3 w-3 text-muted-foreground" /></Button></TooltipTrigger><TooltipContent side="top"><p>Edit Page Title/Note</p></TooltipContent></Tooltip></TooltipProvider>
              )}
            </div>
          </div>
        </CardHeader>
        ...
        <CardFooter className="p-1 grid grid-cols-2 gap-1 border-t">
           <TooltipProvider><Tooltip delayDuration={100}><TooltipTrigger asChild>
            <Button onClick={() => onBranch(panel.id)} size="sm" variant="outline" className="text-[10px] sm:text-xs px-1 py-0.5 h-5 sm:h-6"><GitFork className="mr-1 h-2.5 w-2.5 sm:h-3 sm:w-3" />Branch</Button>
          </TooltipTrigger><TooltipContent side="bottom"><p>Create a new branch from this page.</p></TooltipContent></Tooltip></TooltipProvider>
           <TooltipProvider><Tooltip delayDuration={100}><TooltipTrigger asChild>
            <Button onClick={() => onEditPanel(panel.id)} size="sm" variant="outline" className="text-[10px] sm:text-xs px-1 py-0.5 h-5 sm:h-6"><Edit2 className="mr-1 h-2.5 w-2.5 sm:h-3 sm:w-3" />Edit Page</Button>
  ...
  // Width for this card is controlled by style prop from ReactFlowNode / FlowchartDisplay: clamp(260px, 30vw, 380px)
    ...
    <Card className="w-full h-full overflow-hidden bg-card shadow-md flex flex-col">
      <CardHeader className="p-2 sm:p-3">
        <div className="flex items-center justify-between gap-1 sm:gap-2">
          {isEditingTitle ? (
            <div className="flex-grow flex items-center gap-1">
              <Input type="text" value={editingTitleValue} onChange={(e) => setEditingTitleValue(e.target.value)} onKeyDown={handleTitleKeyDown} onBlur={handleTitleEditSubmit} autoFocus className="h-7 sm:h-8 text-sm w-full flex-grow"/>
              <Button variant="ghost" size="icon" onClick={handleTitleEditSubmit} className="h-6 w-6 sm:h-7 sm:w-7 shrink-0"><Check className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-green-500" /></Button>
              <Button variant="ghost" size="icon" onClick={() => { setIsEditingTitle(false); setEditingTitleValue(panel.title || regularPanelTitle);}} className="h-6 w-6 sm:h-7 sm:w-7 shrink-0"><X className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-red-500" /></Button>
            </div>
          ) : (
            <CardTitle className="text-xs sm:text-sm md:text-base font-semibold text-card-foreground truncate cursor-pointer hover:text-primary flex-grow" title={`Click pencil to edit. ${regularPanelTitle}`} onClick={() => setIsEditingTitle(true)}>
              {regularPanelTitle}
            </CardTitle>
          )}
          <div className="flex items-center shrink-0">
             <TooltipProvider><Tooltip delayDuration={100}>
                <TooltipTrigger asChild>
                    <Button variant="ghost" size="icon" onClick={() => setIsExpanded(!isExpanded)} className="h-6 w-6 sm:h-7 sm:w-7">
                        {isExpanded ? <Minimize2 className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-muted-foreground" /> : <Maximize2 className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-muted-foreground" />}
                    </Button>
                </TooltipTrigger>
                <TooltipContent side="top"><p>{isExpanded ? "Collapse Images" : "Expand Images"}</p></TooltipContent>
            </Tooltip></TooltipProvider>
            {!isEditingTitle && (
              <TooltipProvider><Tooltip delayDuration={100}><TooltipTrigger asChild><Button variant="ghost" size="icon" onClick={() => setIsEditingTitle(true)} className="h-6 w-6 sm:h-7 sm:w-7"><Edit3 className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-muted-foreground" /></Button></TooltipTrigger><TooltipContent side="top"><p>Edit Title</p></TooltipContent></Tooltip></TooltipProvider>
            )}
            {(panel.userDescription || (panel.promptsUsed && panel.promptsUsed.length > 0)) && (
              <TooltipProvider><Tooltip delayDuration={300}><TooltipTrigger asChild><Button variant="ghost" size="icon" className="h-6 w-6 sm:h-7 sm:w-7"><Info className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-muted-foreground" /></Button></TooltipTrigger><TooltipContent className="max-w-[180px] xs:max-w-[200px] sm:max-w-xs break-words bg-popover text-popover-foreground p-1.5 sm:p-2 rounded-md shadow-lg text-xs" side="top">{descriptionContent}</TooltipContent></Tooltip></TooltipProvider>
            )}
          </div>
        </div>
      </CardHeader>
      ...
      <CardFooter className="p-1.5 sm:p-2 md:p-3 grid grid-cols-1 sm:grid-cols-3 gap-1.5 sm:gap-2 border-t">
